 ...
 */
export const createGift = async (name: string, price: number): Promise<Gift> => {
...
export const findAvailableGifts = async (): Promise<Gift[]> => {
    const result = await pool.query('SELECT * FROM gifts WHERE status = $1 ORDER BY name ASC', ['available']);
    return result.rows;
};
 ...
 */
export const reserveGift = async (giftId: number, guestId: number): Promise<Gift> => {
    const result = await pool.query(
 ...
 */
export const confirmGiftPurchase = async (giftId: number, guestId: number): Promise<Gift> => {
    const result = await pool.query(
    ...
    if (result.rows.length === 0) {
        throw new Error('Present not available for purchase or not reserved by this guest');
    }
